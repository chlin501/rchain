...
object GeneratorUtils {
  ...
  def limitedDistinctList[T: Arbitrary](maxSize: Int) =
    distinctListOfGen(arbitrary[T], maxSize = maxSize)(_ == _)

  /**
  Distinct list of elements from a given generator
   with a maximum number of elements to discard
    */
  def distinctListOfGen[T](gen: Gen[T], maxDiscarded: Int = 1000, maxSize: Int = 0)(
      comp: (T, T) => Boolean
  ): Gen[List[T]] = {
    ...
    Gen.sized { s =>
      val size = if (maxSize == 0) s else s % maxSize
